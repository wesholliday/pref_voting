...
def has_truncation_strategy_proofness_violation(prof, vm, set_preference = "single-winner", verbose=False):
    """
    Returns True if there is a voter who can benefit by truncating their preferences (leaving some candidates unranked).
    ...
    for r in ranking_types:
        if not found_manipulator:
            ...
            if isinstance(prof,Profile):
                # For each subset of candidates to keep in the ranking
                for num_to_keep in range(1, len(prof.candidates)):
                    for candidates_to_keep in combinations(prof.candidates, num_to_keep):
                        # Create a new ranking with only the candidates_to_keep
                        new_r_dict = {c: r.index(c) + 1 for c in candidates_to_keep if c in r}
                        new_r = Ranking(new_r_dict)

                        if new_r.rmap != {c: r.index(c) + 1 for c in r if c in candidates_to_keep}:
                            continue  # Skip if the truncation doesn't preserve the original order
                        ...
                        if old_winner_to_compare is not None and r.index(old_winner_to_compare) > r.index(new_winner_to_compare) or (set_preference == "weak-dominance" and r_as_ranking.weak_dom(new_ws,ws)):
                            ...
                            if verbose:
                                print(f"Violation of Truncation-Strategy-Proofness for {vm.name} under the {set_preference} set preference.")
                                print(f"A voter can benefit by truncating their ranking from {r} to {new_r}.")
                                print("")
                                print("Original Profile:")
                                prof.display()
                                print(prof.description())
            ...
            if isinstance(prof,ProfileWithTies):
                ...
                for num_to_keep in range(1, len(prof.candidates)):
                    for candidates_to_keep in combinations(prof.candidates, num_to_keep):
                        # Create a new ranking with only the candidates_to_keep
                        new_r_dict = {c: r_dict[c] for c in candidates_to_keep if c in r_dict}
                        new_r = Ranking(new_r_dict)

                        if new_r.rmap != {c: r_dict[c] for c in r_dict if c in candidates_to_keep}:
                            continue  # Skip if the truncation doesn't preserve the original order
...
def find_all_truncation_strategy_proofness_violations(prof, vm, set_preference = "single-winner", verbose=False):
    """
    Returns a list of tuples (old_ranking, new_ranking) where old_ranking is the original ranking and new_ranking is the truncated ranking that the voter can change to in order to benefit.
    ...
    Returns:
        A List of tuples (old_ranking, new_ranking) where old_ranking is the original ranking and new_ranking is the truncated ranking that the voter can change to in order to benefit.
    """
...
truncation_strategy_proofness = Axiom(
    "Truncation Strategy Proofness",
    has_violation = has_truncation_strategy_proofness_violation,
    find_all_violations = find_all_truncation_strategy_proofness_violations, 
)
