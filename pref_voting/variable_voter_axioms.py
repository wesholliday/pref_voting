...
def has_truncated_involvement_violation(prof, vm, verbose=False):
    """
    Returns True if there is a ballot such that starting from an updated version of prof 
    with the ballot removed, adding a truncated version of the ballot causes the winners 
    to shift from candidates ranked by the truncated ballot to candidates unranked by the
    truncated ballot.
    
    In other words, adding a truncated ballot to a profile shifts the winners from being 
    entirely among the candidates ranked by the ballot to entirely among the candidates 
    unranked by the ballot.
...
def has_positive_truncated_involvement_violation(prof, vm, verbose=False):
    """
    Returns True if there is a ballot such that starting from an updated version of prof 
    with the ballot removed, adding a truncated version of the ballot causes the winners 
    to shift from the top-ranked candidate plus possibly other ranked candidates to being 
    entirely among the unranked candidates.
    
    In other words, adding a truncated ballot to a profile shifts the winners from including 
    the top-ranked candidate (plus possibly other ranked candidates) to being entirely among 
    the candidates unranked by the ballot.
        ...
        for r_idx, r in enumerate(prof.rankings):
            ...
            for truncation_level in range(1, len(prof.candidates)):
                ...
                if winners_include_top_ranked and winners_from_unranked:
                    if verbose:
                        print(f"Positive Truncated Involvement violation found:")
                        print("Original profile:")
                        prof.display()
                        print(prof.description())
                        vm.display(prof)
...
def find_all_positive_truncated_involvement_violations(prof, vm, verbose=False):
    """
    Returns all violations of positive truncated involvement for a given profile and voting method.
...
positive_truncated_involvement = Axiom(
    "Positive Truncated Involvement",
    has_violation = has_positive_truncated_involvement_violation,
    find_all_violations = find_all_positive_truncated_involvement_violations,
)
...
variable_voter_axioms = [
    ...
    positive_truncated_involvement,
]
